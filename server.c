...
// Создание процесса child, который использует ранее созданные pipe для STDIN и STDOUT.
void CreateChildProcess();

// Читаем из файла и записываем содерживомое в pipe for the child's STDIN.
// Останавливаемся, когда все считаем.
DWORD WINAPI WriteToPipe(LPDWORD dummy);

// Читаем output из child process's pipe for STDOUT
// и записываем в parent process's pipe for STDOUT. 
// Останавливаемся, когда все считаем.
...
{
    WSADATA wsaData; // Метаданные сети
    int iResult;
    ...
    // генерация PIPE и CMD
    SECURITY_ATTRIBUTES saAttr;

    printf("\n->Start of parent execution.\n");

    // Установка флага bInheritHandle, чтобы дескрипторы каналов наследовались.
    saAttr.nLength = sizeof(SECURITY_ATTRIBUTES);
    saAttr.bInheritHandle = TRUE;
    saAttr.lpSecurityDescriptor = NULL;

    // Создаем pipe для child process's STDOUT. 
    if (!CreatePipe(&g_hChildStd_OUT_Rd, &g_hChildStd_OUT_Wr, &saAttr, 0))
        ErrorExit(TEXT("StdoutRd CreatePipe"));

    // Убеждаемся, что дескриптор чтения pipe для STDOUT не наследуется.
    if (!SetHandleInformation(g_hChildStd_OUT_Rd, HANDLE_FLAG_INHERIT, 0))
        ErrorExit(TEXT("Stdout SetHandleInformation"));

    // Создаем pipe для child process's STDIN. 
    if (!CreatePipe(&g_hChildStd_IN_Rd, &g_hChildStd_IN_Wr, &saAttr, 0))
        ErrorExit(TEXT("Stdin CreatePipe"));

    // Убеждаемся, что дескриптор писания pipe для STDIN не наследуется.
    if (!SetHandleInformation(g_hChildStd_IN_Wr, HANDLE_FLAG_INHERIT, 0))
        ErrorExit(TEXT("Stdin SetHandleInformation"));

    // Создаем процесс child. 
    CreateChildProcess();
    ...
    // закроем соединение, как мы закончим
